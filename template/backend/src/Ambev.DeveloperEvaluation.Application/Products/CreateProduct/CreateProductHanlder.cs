    ...
    public async Task<CreateProductResult> Handle(CreateProductCommand command, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (existingProduct != null)
                return new CreateProductResult { Description = $"[Business Rule] Product already exists : {command.Title}" };
        ...
        {
            throw new InvalidOperationException($"Error create product: {ex.InnerException }");
        }
    }
}